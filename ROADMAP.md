...
Phase 1 is locally complete. The first candidate changes only HLS preparation: bounded reuse of finalized video/audio, parallel text extraction, event-driven process completion and cancellation of a redundant source-demand scan. The strict gate passed, and 40 short-fixture preparations preserve baseline output byte for byte. Phase 2 has also passed the strict gate and 32 comparisons using a 2.218 GB input: warm reuse saves about 0.75 seconds of local preparation, while first admission adds about 0.03 to 0.05 seconds compared with the baseline. The report records CPU, sampled memory, disk use and measurement limits. An isolated eight-phase cache test is prepared. Apple TV startup gains are not established. Direct MP4 and bitmap OCR scheduling remain unchanged. The receiver matrix, end-to-end timing, external-storage measurements and publication acceptance remain pending. Larger engine and streaming changes follow measurements, not this initial scope.

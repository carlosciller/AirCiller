...
The current candidate coalesces that four-input burst into one final seek. The check requires exactly one correlated acknowledgement for the burst and a matching receiver position. The post-seek observation lasts eight seconds; receipt of a playing event does not establish immediate subtitle visibility. Earlier validation records describe the implementation tested at their respective dates.

For isolated subtitle diagnosis, a private control plan can specify `"profile": "subtitleSeek"`. It requires one HLS clip lasting 100 to 180 seconds with an external subtitle. After initial playback, it pauses, seeks once to second 15, resumes and observes for twelve seconds; it repeats at seconds 2 and 88 with eight-second observations. This separates forward/backward seeking from a burst and allows a controlled 90-second cue to clear. It retains the same authorization, capture-source and cleanup rules. It is not a built-in fixture in the standard capture configuration; subtitle content and output assessment must match the supplied private fixture.
...
| `hlsCacheReuse` | Use a disposable HLS cache and check eight separately captured phases: first preparation, replay, subtitle replacement, subtitle delay, alternate audio, audio delay, original audio restored, and subtitles off. Require expected cache hits/misses, unchanged base-file hashes on hits, receiver starts and the matching subtitle/audio in every phase. |
...
`hlsCacheReuse` uses that dual-audio fixture. It requires misses for the first preparation, alternate audio and audio-delay change; the other five phases must hit the cache. Its video/audio hash inventory is bounded to 256 MiB and read after receiver confirmation. These reads and the output observations make this an integrity test, not a clean startup benchmark. The private report distinguishes the action request, preparation trace and receiver-confirmation timestamps; none is called the first visible frame. The capture evaluator also requires successful cleanup of the isolated cache. The delay phases check reuse/invalidation and continued output, not the physical accuracy of the requested time offset.

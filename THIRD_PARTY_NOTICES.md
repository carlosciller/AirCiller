...
| [python-build-standalone](https://github.com/astral-sh/python-build-standalone) | Redistributable CPython runtime included with AirCiller | MPL-2.0 build tooling; bundled CPython and library licenses remain applicable |
...
AirCiller includes a self-contained FFmpeg build compiled from the official FFmpeg 9.0.1 source with no optional third-party codec libraries. Its LGPL-2.1 notice is included in the app. The exact source URL, checksum and configure options are recorded in `Scripts/build_managed_components.sh`.

The included Python runtime is derived from Astral's `python-build-standalone` CPython 3.13.15 distribution. Its upstream SHA-256 digest is pinned in `Scripts/build_managed_components.sh`; the upstream distribution includes CPython and bundled-library license texts.

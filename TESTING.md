...
The [0.12.5 external bitmap record](Docs/EXTERNAL_VOBSUB.md) covers paired VobSub discovery, languages, damaged input, cache invalidation, local timing and corrected Apple TV output checks. It retains incomplete assessments, initial capture failures and the MP4 language-tag failure separately. [External PGS and rapid Mac seeks](Docs/EXTERNAL_PGS.md) have their own preceding validation record.
...
The user subsequently confirmed picture, sound and selectable subtitles on the corrected direct Dolby Vision candidate, and repeated pause/resume plus forward/backward commands from both the physical Apple TV remote and iPhone Remote. AirCiller remained synchronized and connected. The planned physical checks for this review are complete. See [the review record](Docs/STABILITY_REVIEW.md) for evidence and remaining limitations.

After acceptance, 0.12.1 (build 54) passed the complete local suite again. The playback source and locked dependencies match the physically tested code in `6a46dbe`; the release changes version metadata and documentation only. Packaging and signed-update verification remain separate distribution checks. The daily-use installed app is unchanged.
